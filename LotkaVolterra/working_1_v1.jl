...
res1 = Optimization.solve(optprob, Adam(0.1), callback=callback, maxiters = 500)
...
# Plot the losses
pl_losses = plot(1:500, losses[1:500], yaxis = :log10, xaxis = :log10, xlabel = "Iterations", ylabel = "Loss", label = "ADAM", color = :blue)
plot!(500:length(losses), losses[500:end], yaxis = :log10, xaxis = :log10, xlabel = "Iterations", ylabel = "Loss", label = "BFGS", color = :red)
...
p1 = plot(t,abs.(Array(solution) )' .+ eps(Float32),
          lw = 3, yaxis = :log, title = "Timeseries of UODE Error",
          color = [3 :orange], xlabel = "t",
          label = ["x(t)" "y(t)"],
          titlefont = "Helvetica", legendfont = "Helvetica",
